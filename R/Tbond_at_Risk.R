...
# data loading
StarsDataM <- readRDS('Output/macro_data.rds')[["monthly"]]

# Step1: variable selection  : types - growth, level, etc.
variable_type <- names(StarsDataM) %>% 
                    str_subset("_MG$|INT_|RATIO|RATE|.+2.+|BAL|VOL|^US_T")

# selected variables and remove linear dependent varialbes
data_selected0 <- StarsDataM %>% 
                  select(all_of(c("yearM",variable_type)) %>% sort())  

## Step2 : Handling NAs (no observations)      

start_yearM <- "2005-01" %>% as.yearmon(format="%Y-%m") %>% as.numeric()
end_yearM <- "2022-4" %>% as.yearmon(format="%Y-%m") %>% as.numeric()


var_na <- data_selected0 %>% 
          filter(yearM>=start_yearM, yearM<=end_yearM) %>% 
          is.na() %>% 
          colSums() %>% 
          .[.>0]
var_na
...
# selected variables
data_selected <- data_selected0 %>% select(yearM,!any_of(names(var_na))) %>% 
                  filter(yearM>=start_yearM, yearM<=end_yearM) 
# check
...
ggplot(data_scaledL,aes(yearM,value))+
...
# fitted value 
ggplot(pca_data_aug, aes(.fittedPC1, .fittedPC2,color=US_TNote10Y_M)) + 
  geom_point(size = 2) +
...
quant_data <- pca_data_aug %>% 
              select(yearM, all_of(c(target,components)))
quant_dataL <- quant_data %>% 
               pivot_longer(-yearM) %>% 
               mutate(vartype=ifelse(str_detect(name,"^PC"),"predictors","responses"))

ggplot(quant_dataL,aes(yearM,value,group=name,color=name))+
  geom_line()+
...
#var <- "US_TNote10Y_M";pred_terms=8;quant_data=quant_data;components=components;tau=c(0.1,0.5,0.9)
  
  
    
Res <- foreach(lead_period=1:pred_terms,.combine=rbind) %do% {

  #var <- "US_TNote10Y_M";lead_period=1;quant_data=quant_data;components=reg_component[[var]];tau=c(0.1,0.5,0.9)
  ...
  fitted_data_aug <- reduce(fitted_data,
                            .f=function(.x,.y) 
                               left_join(.x,.y,by=c("yearM",var,components))) %>%
                     set_names(c("yearM", attr(attr(terms(formula),"factors"),"dimnames")[[1]],as.character(tau)))

  # ggplot(fitted_data_aug,aes(yearM,`0.5`))+
  #   geom_line()+
...
toOriginScale <-{(scaling_info %>% filter(name==var) %>% pull(sd))*Res[,1:length(tau)]+
                ...
                as_tibble() %>% 
                mutate(yearM=as.yearmon(seq(as.Date(pull(quant_data,yearM) %>% last()),
                                            by="month",length.out=pred_terms+1)[-1])) %>% 
                select(yearM,everything())
...
ggplot(tidy_results,aes(yearM,`0.5`))+
   geom_line()+
